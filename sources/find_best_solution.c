...
void		ft_solve3(t_push **node1, t_output **out, int iters)
{
	int		i;
	t_push	*node2;
	...
	if ((*node1)->debug_opt & DEBUG)
		ft_printf("End of solution #%d\n", (iters == 1 ? 2 : 3));
}

void		ft_solve2(t_push **node1, t_output **out)
{
	int		buf;
	t_push	*node2;
	int		swapper;
...
t_output	**best_solution(t_push **node1)
{
	...
	ft_solve2(node1, &out[0]);
	iterations = 1;
	ft_solve3(&node1_2, &out[1], iterations);
	iterations = 3;
	ft_solve3(&node1_3, &out[2], iterations);
	free_node(node1_2);
	free_node(node1_3);
	return (out);
}
